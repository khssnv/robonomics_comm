...
def ipfs_download(multihash):
    ...
    download = rospy.ServiceProxy('/liability/infochan/ipfs/get_file', IpfsDownloadFile) 
...
def ipfs_upload(messages):
    ...
    upload = rospy.ServiceProxy('/liability/infochan/ipfs/add_file', IpfsUploadFile) 
    with NamedTemporaryFile(delete=False) as tmpfile:
...
class Objective:
    def __init__(self, messages=None, multihash=None):
        ...
        if messages is None and multihash is None:
            raise NotImplementedError('messages or objective should be set')
